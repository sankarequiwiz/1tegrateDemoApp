...
const SelectIncident = React.forwardRef<HTMLDivElement, HTMLProps<HTMLDivElement>>((props, ref) => {
   const { integration, setCurrentStep, setSelectedService,domain,current, selectedCollection,selectedService,selectedOrganization = 'default' } = React.useContext(AppContext);
   ...
   const getAllTickets = async () => {
      setLoading(true);
      try {
         const resp = await API.services.getIncidentOpsigine(selectedOrganization, selectedCollection, selectedService,{ integrationId: integration?.id } ,domain);
   ...
   return (
      ...
      >
         ...
         <Footer
            onCancel={() => setCurrentStep(current - (selectedService === 'default' ? 2 : 1))}
